...
def dirb(ip_address, port, url_start):
    ...
    DIRBSCAN = "dirb %s://%s:%s -o reports/%s/dirb-%s.txt -r" % (url_start, ip_address, port, ip_address, ip_address)
...
def mssqlEnum(ip_address, port):
    ...
    MSSQLSCAN = "nmap -sV -Pn -p %s --script=ms-sql-info,ms-sql-config,ms-sql-dump-hashes --script-args=mssql.instance-port=1433,smsql.username-sa,mssql.password-sa -oN reports/%s/mssql_%s.nmap %s" % (port, ip_address, ip_address)
...
def smbEnum(ip_address, port):
    print "INFO: Detected SMB on " + ip_address + ":" + port
    enum4linux = "enum4linux -a %s > reports/%s/enum4linux_%s" % (ip_address, ip_address, ip_address)
...
def ftpEnum(ip_address, port):
    ...
    FTPSCAN = "nmap -sV -Pn -vv -p %s --script=ftp-anon,ftp-bounce,ftp-libopie,ftp-proftpd-backdoor,ftp-vsftpd-backdoor,ftp-vuln-cve2010-4221 -oN 'reports/%s/ftp_%s.nmap' %s" % (port, ip_address, ip_address, ip_address)
...
def udpScan(ip_address):
    ...
    UDPSCAN = "nmap -vv -Pn -A -sC -sU -T 4 --top-ports 200 -oN 'reports/%s/udp_%s.nmap' %s"  % (ip_address, ip_address, ip_address)
    ...
    UNICORNSCAN = "unicornscan -mU -v -I %s > reports/%s/unicorn_udp_%s.txt" % (ip_address, ip_address, ip_address)
